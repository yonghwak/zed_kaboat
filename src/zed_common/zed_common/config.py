...
# ---- 미션5 목표유도 + gap follower 회피 ----
AVOID_FOV_DEG = 100.0            # 전방 스캔 범위(좌우 절반씩)
AVOID_SECTORS = 7                # 이 범위를 몇 구간으로 나눠서 볼지
AVOID_MAX_RANGE_M = 6.0          # 이보다 먼 포인트는 무시(그 구간은 "완전히 열림"으로 봄)
AVOID_MIN_GAP_CLEARANCE_M = 2.5  # 이 거리 이상 뚫려있어야 "열린 구간(gap)"으로 인정
AVOID_LINEAR_MAX = 0.5
AVOID_KP_ANGULAR = 1.3
...
TARGET_CANDIDATE_TIMEOUT_SEC = 4.0  # 이 시간 동안 재감지 안 되면 후보 폐기POSE_TOPIC = "/zed/zed_node/pose"
MAP_FRAME = "map"
